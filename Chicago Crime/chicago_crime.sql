...
SELECT 
  * 
FROM 
  practice..chicago_crime 
  
-- Looking total beat vs total ward based on primary type
SELECT 
  primary_type, 
  SUM(beat) AS total_beat, 
  SUM(ward) AS total_ward 
FROM 
  practice..chicago_crime 
GROUP BY 
  primary_type 
...
SELECT 
  primary_type, 
  year, 
  MAX(community_area) AS max_community_area 
FROM 
  practice..chicago_crime 
WHERE 
  year >= '2010' 
...
SELECT 
  year, 
  arrest, 
  SUM(community_area) AS total_community_area 
FROM 
  practice..chicago_crime 
GROUP BY 
  year, 
...
SELECT 
  ...
  location_description, 
  community_area, 
  CASE WHEN community_area > 50 THEN 'Very Large' WHEN community_area = 50 THEN 'Large' ELSE 'Very Small' END AS community_area_description 
FROM 
  practice..chicago_crime 
ORDER BY 
  primary_type ASC 
...
FROM 
  practice..chicago_crime 
WHERE 
  primary_type IN ('THEFT', 'ASSAULT', 'BATTERY') 
GROUP BY 
  primary_type, 
  description, 
  location_description 
