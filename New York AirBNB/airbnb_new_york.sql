...
select 
  neighbourhood_group, 
  count(neighbourhood_group) as value 
from 
  practice..AB_NYC_2019 
group by 
  neighbourhood_group 
...
from 
  practice..AB_NYC_2019 
where 
  room_type in ('Private room', 'Shared room') 
...
select 
  ...
  minimum_nights, 
  (
    sum(price) * minimum_nights
  ) as total_price 
from 
  practice..AB_NYC_2019 
where 
  room_type = 'Private room' 
  and neighbourhood_group in ('Queens', 'Staten Island') 
group by 
  name, 
  room_type, 
  neighbourhood_group, 
...
select 
  ...
  neighbourhood, 
  room_type, 
  avg(price) as avg_price_per_day, 
  avg(
    (price) * 30
  ) as avg_price_per_month 
from 
