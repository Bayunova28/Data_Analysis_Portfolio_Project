...
SELECT CourseName, ItemDescription, LMPField, SUM(((ActualPrice * Quantity) - SalesTax) * 1000) AS SalesPrice
FROM practice..park_golf_sales
GROUP BY CourseName, ItemDescription, LMPField
HAVING SUM(((ActualPrice * Quantity) - SalesTax) * 1000) >= 10000
ORDER BY CourseName ASC

-- Looking sales price description based on course name, item description and LMP
SELECT CourseName, ItemDescription, LMPField, SUM(((ActualPrice * Quantity) - SalesTax) * 1000) AS SalesPrice,
CASE 
	WHEN SUM(((ActualPrice * Quantity) - SalesTax) * 1000) > 500000 THEN 'Very Expensive'
	WHEN SUM(((ActualPrice * Quantity) - SalesTax) * 1000) = 500000 THEN 'Intermediate'
	ELSE 'Cheaper'
END AS sales_price_description
FROM practice..park_golf_sales
GROUP BY CourseName, ItemDescription, LMPField
ORDER BY CourseName ASC
...
SELECT CourseName, ItemDescription, LMPField, SUM(((ActualPrice * Quantity) - SalesTax) * 1000) AS SalesPrice
FROM practice..park_golf_sales
WHERE LMPField IN ('GolfPass', 'GolfSpecial', 'FootGolfWD', 'FootGolfWE')
GROUP BY CourseName, ItemDescription, LMPField
ORDER BY CourseName ASC
