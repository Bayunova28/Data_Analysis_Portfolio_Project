...
FROM practice..center_medicare_service
ORDER BY nppes_provider_first_name ASC
...
FROM practice..center_medicare_service
ORDER BY nppes_provider_city ASC
...
SELECT nppes_provider_last_org_name, nppes_provider_state, SUM(total_claim_count * 30) AS total_claim_count_per_month, 
SUM(total_day_supply) AS total_day_supply_per_month 
FROM practice..center_medicare_service
GROUP BY nppes_provider_last_org_name, nppes_provider_state
HAVING SUM(total_day_supply) <= 500
ORDER BY nppes_provider_last_org_name ASC
...
SELECT nppes_provider_city, nppes_provider_state, specialty_description, drug_name, generic_name, total_day_supply,
CASE 
	WHEN total_day_supply > 5000 THEN 'Very Enough'
	WHEN total_day_supply = 5000 THEN 'Enough'
	ELSE 'Very Less'
...
SELECT nppes_provider_last_org_name, nppes_provider_city, specialty_description, total_day_supply
FROM practice..center_medicare_service
WHERE specialty_description IN ('Clinic/Center', 'Peripheral Vascular Disease', 'Pharmacist')
ORDER BY nppes_provider_last_org_name ASC
